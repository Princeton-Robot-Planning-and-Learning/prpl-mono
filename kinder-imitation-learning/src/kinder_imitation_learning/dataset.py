#!/usr/bin/env python
"""Dataset utilities for KinDER imitation learning."""
...
def load_teleop_demonstrations(
    ...
    if render_images:
        # Add kinder src to path if not already there
        kinder_root = teleop_data_dir.parent.parent.parent
        kinder_src = kinder_root / "src"
        if str(kinder_src) not in sys.path:
            sys.path.insert(0, str(kinder_src))

        try:
            # Register all kinder environments
            kinder.register_all_environments()
        except ImportError as e:
            raise ImportError(
                f"Failed to import kinder/gymnasium for rendering: {e}\n"
                f"Tried adding {kinder_src} to path. Make sure kinder is installed."
            ) from e
