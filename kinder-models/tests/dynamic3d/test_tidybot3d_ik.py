...
def test_ik_solver_performance_and_accuracy():
    ...
    home_pos = np.array([0.576, 0.0, 0.434])
    ...
    result_qpos = ik_solver.solve(retract_pos, retract_quat, retract_qpos)
    assert result_qpos.shape == retract_qpos.shape
    assert np.all(np.isfinite(result_qpos))

    assert np.allclose(
        result_qpos, retract_qpos, atol=5
    ), f"IK solution deviates from expected: {result_qpos} vs {retract_qpos}"
