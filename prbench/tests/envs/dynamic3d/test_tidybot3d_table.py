...
def test_tidybot_table_clutter_pick_place_goals():
    ...
    env = ObjectCentricTidyBot3DEnv(
        scene_type="table",
        num_objects=7,
        task_config_path=str(tasks_root / "tidybot-table-o7-clutterPickPlace.json"),
        render_images=False,
    )
    ...
    assert not env.reward(obs), "Goals should not be satisfied after reset"

    # Get the current state
    current_state = env._get_current_state()

    # Get all objects and the table fixture
    table = list(env._fixtures_dict.values())[0]  # Assume first fixture is the table
    object_names = [obj.name for obj in env._objects_dict.values()]

    # Get goal regions from the table
    goal_regions = table.regions.get("goal", [])
    if goal_regions:
        ...
        for i, obj_name in enumerate(object_names):
            ...
            goal_z = table.table_height + 0.05  # Slightly above table surface

            # Update the state with new object position
            modified_state = modified_state.set(obj, "x", goal_x)
            modified_state = modified_state.set(obj, "y", goal_y)
            modified_state = modified_state.set(obj, "z", goal_z)
