...
These tests verify that the PD controller correctly converts target joint
positions to torques and that the arm behaves reasonably under control.
"""
...
def test_pd_controller_computes_torques(env):
    ...
    assert np.all(np.abs(torques) <= robot_env.ARM_TORQUE_LIMITS), (
...
def test_torque_saturation(env):
    ...
    assert np.all(np.abs(torques) <= limits + 0.01), (
...
def test_arm_responds_to_target(env):
    ...
    This is a basic test that the PD controller is actually influencing
    the arm motion, without requiring perfect convergence.
    """
    env.reset(seed=42)
    ...
    assert np.any(position_change > 0.001), (
...
def test_arm_converges_to_target_position():
    ...
    try:
        ...
        assert np.all(np.abs(final_arm_pos) < 10), (
...
def test_gravity_compensation_is_applied():
    ...
    try:
        ...
        assert np.max(np.abs(gravity_comp)) > 0.1, (
...
def test_arm_remains_stable():
    ...
    try:
        ...
        for step in range(200):
            ...
            assert not np.any(np.isnan(current_pos)), (
                f"NaN in arm position at step {step}: {current_pos}"
            )
            assert not np.any(np.isinf(current_pos)), (
