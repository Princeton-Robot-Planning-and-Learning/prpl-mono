...
from pybullet_helpers.geometry import Pose, multiply_poses, get_pose
...
def test_pick_place_one_obstructions():
    """Test that picking and placing succeeds when there are one obstructions."""
    # Create the real environment.
    env = Obstruction3DEnv(num_obstructions=1, use_gui=True, render_mode="rgb_array")
    assert isinstance(env.observation_space, ObjectCentricBoxSpace)
    config = env._object_centric_env.config  # pylint: disable=protected-access
    if MAKE_VIDEOS:
        env = RecordVideo(env, "unit_test_videos")
    ...
    # Create a simulator for planning.
    sim = ObjectCentricObstruction3DEnv(num_obstructions=1, config=config)
    sim.reset()
    sim.set_state(obs)
    ...
    # Helper function to execute a pick-and-place for a given object.
    def pick_and_place_object(
        object_name, placement_x, placement_y
    ):
        nonlocal obs, vec_obs

        # First, move to pre-grasp pose (top-down).
        object_id = sim._object_name_to_pybullet_id(object_name)
        object_pose = get_pose(object_id, sim.physics_client_id)
        object_half_extents = sim._get_half_extents(object_name)
        ...
        # The object should now be grasped.
        assert obs.grasped_object == object_name
        ...
        placement_padding = 1e-4  # leave some room to prevent collisions with surface
        # Place the obstruction to the side of the target region.
        # Get the table height from the current pose.
        table_height = obs.target_region_pose.position[2] - obs.target_region_half_extents[2]
        block_placement_pose = Pose(
            (
                placement_x,
                placement_y,
                table_height + object_half_extents[2] + placement_padding,
            ),
            obs.target_region_pose.orientation,
        )
        end_effector_placement_pose = multiply_poses(
        ...
        # Motion plan to the placement location.
        sim.set_state(obs)
        current_end_effector_pose = sim.robot.get_end_effector_pose()
        joint_plan = smoothly_follow_end_effector_path(
            sim.robot,
    ...
    # Pick and place the first obstruction.
    ...
    # # Pick and place the second obstruction.
    # pick_and_place_object(
    #     "obstruction1",
    #     placement_x=0.3,
    #     placement_y=-0.3,
    ...
    # Now pick and place the target block.
    # First, move to pre-grasp pose (top-down).
    x, y, z = obs.target_block_pose.position
    dz = 0.025
    pre_grasp_pose = Pose.from_rpy((x, y, z + dz), (np.pi, 0, np.pi / 2))
    ...
    # Motion plan to place the target block.
    sim.set_state(obs)
    current_end_effector_pose = sim.robot.get_end_effector_pose()
    joint_plan = smoothly_follow_end_effector_path(
        sim.robot,
