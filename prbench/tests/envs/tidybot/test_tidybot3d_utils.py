...
class TestGetTableBbox:
    """Tests for get_table_bbox function."""

    def test_rectangle_table_bbox(self):
    ...
    def test_circle_table_bbox(self):
...
class TestBboxesOverlap:
    ...
    def test_touching_boxes_no_margin(self):
...
class TestSampleCollisionFreePosition:
    ...
    def test_no_existing_tables(self):
        ...
        pos = sample_collision_free_position(
            self.table_config, placed_bboxes, self.np_random
        )
    ...
    def test_with_existing_tables(self):
        ...
        new_bbox = get_table_bbox(pos, self.table_config)
    ...
    def test_deterministic_with_seed(self):
        ...
        pos1 = sample_collision_free_position(self.table_config, placed_bboxes, rng1)

        # Sample with second generator with same seed
        rng2 = np.random.default_rng(123)
        pos2 = sample_collision_free_position(self.table_config, placed_bboxes, rng2)

        np.testing.assert_array_equal(pos1, pos2)

    def test_crowded_scenario_fallback(self):
        ...
        pos = sample_collision_free_position(
            large_table_config, placed_bboxes, self.np_random, max_attempts=5
        )
...
class TestSamplePoseInRegion:
    ...
    def test_single_region(self):
        ...
        x, y, z = sample_pose_in_region(regions, self.np_random)
    ...
    def test_custom_z_coordinate(self):
        """Test sampling with custom z coordinate."""
        regions = [[0.0, 0.0, 1.0, 1.0]]
        custom_z = 0.5

        x, y, z = sample_pose_in_region(regions, self.np_random, z_coordinate=custom_z)
    ...
    def test_empty_regions_list(self):
        ...
        with pytest.raises(ValueError, match="Regions list cannot be empty"):
            sample_pose_in_region(regions, self.np_random)
