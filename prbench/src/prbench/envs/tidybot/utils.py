...
def get_table_bbox(pos: NDArray[np.float32], table_config: dict[str, Any]) -> list[float]:
...
def bboxes_overlap(bbox1: list[float], bbox2: list[float], margin: float = 0.2) -> bool:
    ...
    """
    return not (bbox1[2] + margin <= bbox2[0] or  # bbox1 right + margin <= bbox2 left
              bbox2[2] + margin <= bbox1[0] or  # bbox2 right + margin <= bbox1 left
              bbox1[3] + margin <= bbox2[1] or  # bbox1 top + margin <= bbox2 bottom
              bbox2[3] + margin <= bbox1[1])    # bbox2 top + margin <= bbox1 bottom
