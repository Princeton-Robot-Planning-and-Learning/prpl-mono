...
class MujocoEnv(gymnasium.Env[MjObs, MjAct]):
    ...
    def _set_object_pos_quat(
        ...
        new_qpos = self.sim.data.qpos.at[joint_id : joint_id + 7].set(
            np.array([float(x) for x in pos] + [float(q) for q in quat])
        )
        self.sim.data.replace(qpos=new_qpos)
    ...
    def get_object_pos_quat(self, name: str) -> tuple[float, float]:
        ...
        assert self.sim is not None, "Simulation not initialized"
        joint_id = self.sim.model.get_joint_qpos_addr(f"{name}_joint")
        pos = self.sim.data.qpos.at[joint_id : joint_id + 3]
        quat = self.sim.data.qpos.at[joint_id + 3 : joint_id + 7]
        return pos, quat
...
class MjxModel:
    ...
    def __init__(self, xml_string: str) -> None:
        ...
        self._model: mujoco.MjModel = mujoco.MjModel.from_xml_string(xml_string)
        self._mjx_model: mjx.MjModel = mjx.put_model(
            self._model, impl="jax"
...
class MjSim:
    ...
    def __init__(self, xml_string: str, camera_width: int, camera_height: int) -> None:
        ...
        self.model: MjxModel = MjxModel(xml_string)
        self.data: mjx.MjData = self._make_data(
            impl="jax", nconmax=12 * 8192, njmax=96
        )  # TODO make impl configurable
    ...
    def _make_data(self, impl=None, nconmax=None, njmax=None, device=None) -> None:
        """Create the MjData object for the simulation."""
        return mjx.make_data(
            self.model._model, impl=impl, nconmax=nconmax, njmax=njmax, device=device
        )
