...
from prbench_models.geom3d.obstruction3d.parameterized_skills import (
...
def test_pick_controller():
    ...
    env = prbench.make(
        "prbench/Obstruction3D-o1-v0", render_mode="rgb_array", use_gui=False, realistic_bg=True
    )
    if MAKE_VIDEOS:
...
def test_pick_place_controller():
    ...
    env = prbench.make(
        "prbench/Obstruction3D-o0-v0", render_mode="rgb_array", use_gui=False, realistic_bg=True
    )
    if MAKE_VIDEOS:
        env = RecordVideo(env, "unit_test_videos", name_prefix="Obstruction3D")
    ...
    sim = ObjectCentricObstruction3DEnv(num_obstructions=0)
    controllers = create_lifted_controllers(
        env.action_space,
        sim,
    )
