...
def success_average_reward(rewards):
    success_rewards = []
    for reward in rewards:
        if reward != -500:
            success_rewards.append(reward)
    return np.mean(success_rewards)
...
def main():
    ...
    with open(args.train_results_dir / "info.json", "r") as f:
        train_data = json.load(f)
    with open(args.eval_results_dir / "multi_seed_eval_results.json", "r") as f:
    ...
    for seed_data in eval_data["seeds"]:
        seed_data_detailed[seed_data["seed"]] = {
            ...
            "Steps": [-step for step in seed_data["full_info"]['per_task'][0]['metrics']['sum_rewards']],
        }
    ...
    new_eval_data = {
        "summary statistics": {
            ...
            "wall clock time per episode": eval_data["aggregated_metrics"]["wall_clock_time_per_episode"],
    ...
    with open(f"outputs/alpha_imitation_learning/{task_name}/organized_results.json", "w") as f:
